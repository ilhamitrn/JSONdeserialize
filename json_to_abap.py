...
class AbapStructureGenerator:
    ...
    def _build_full_report_code(self, root_name, original_json):
        """Generates a directly executable Z-Report code."""
        
        # --- CRITICAL FIXES FOR ABAP STRING LITERALS ---
        # 1. Remove newlines (\n, \r) because ABAP string literals cannot span lines.
        # 2. Escape single quotes (' -> '') because ' starts/ends a string in ABAP.
        
        clean_json = original_json.replace('\n', '').replace('\r', '') 
        escaped_json = clean_json.replace("'", "''")                  
        
        # Split into 200-char chunks to be safe with ABAP editor width
        chunk_size = 200
        ...
        final_code += "*& Generated by ABAP Deep Structure Generator (Python Tool)\n"
        final_code += "*&---------------------------------------------------------------------*\n"
        final_code += "REPORT z_json_deserialize_test.\n\n"
...
* --- 3. BUILD JSON DATA ---
"""
        # Build JSON string using CONCATENATE
...
* --- 6. DISPLAY RESULT ---
  IF gs_data IS INITIAL.
    WRITE: / 'ERROR: Data is empty! Check mapping table or JSON format.'.
  ELSE.
